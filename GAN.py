...
def build_model():
    ...
    # 或使用交叉熵损失
    '''
    d_value_real = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=d_outputs_real_logits,
                                                                         labels=tf.ones_like(d_outputs_real_logits)))
    d_value_fake = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=d_outputs_fake_logits,
                                                                         labels=tf.zeros_like(d_outputs_fake_logits)))
    d_value = d_value_fake + d_value_real

    g_value = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=d_outputs_fake_logits,
                                                                    labels=tf.ones_like(d_outputs_fake_logits)))
    '''
    # 优化
